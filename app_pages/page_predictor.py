...
def page_predictor_body():
    ...
    with col1:
        ...
        income = st.slider("Annual Income (USD)",
                           min_value=20000, max_value=150000,
    ...
    with col2:
        ...
        campaign_type = st.selectbox(
            "Campaign Type",
            options=["Awareness", "Consideration",
                     "Conversion", "Retention"])
        ad_spend = st.slider("Ad Spend (USD)",
                             min_value=0.0, max_value=10000.0,
                             value=500.0, step=50.0)
        ctr = st.slider("Click-Through Rate",
                        min_value=0.0, max_value=0.5,
                        value=0.05, step=0.01)

    with col3:
        st.markdown("**Engagement Metrics**")
        website_visits = st.slider("Website Visits",
                                   min_value=0, max_value=50, value=5)
        pages_per_visit = st.slider("Pages Per Visit",
                                    min_value=1.0, max_value=20.0,
                                    value=3.0, step=0.5)
        time_on_site = st.slider("Time on Site (min)",
                                 min_value=0.0, max_value=60.0,
                                 value=5.0, step=0.5)
        social_shares = st.slider("Social Shares",
                                  min_value=0, max_value=100, value=5)
        email_opens = st.slider("Email Opens",
                                min_value=0, max_value=30, value=3)
        email_clicks = st.slider("Email Clicks",
                                 min_value=0, max_value=20, value=1)
        previous_purchases = st.slider("Previous Purchases",
                                       min_value=0, max_value=10, value=1)
        loyalty_points = st.slider("Loyalty Points",
        ...
        try:
            ...
            with col_pred:
                if prediction == 1:
                    ...
                    st.markdown("This lead is predicted to **convert**.")
                else:
                    st.error("## ✘ NOT CONVERTED")
                    st.markdown(
                        "This lead is predicted **not to convert**.")
